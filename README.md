...
- Windows 下用当前 `node.exe` 直接跑同目录的 `node_modules/npm/bin/npm-cli.js`，不 spawn `npm.cmd`。找不到 JS 入口时才退回 `cmd.exe`：整条命令外层再包一层引号，并用 `windowsVerbatimArguments`，避免 Node 把引号转成 `\"`
